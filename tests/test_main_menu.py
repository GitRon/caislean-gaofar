...
def test_load_save_files_empty(main_menu):
    ...
    with patch("main_menu.SaveGame.list_save_files", return_value=[]):
...
def test_load_save_files_with_data(main_menu):
    ...
    with patch("main_menu.SaveGame.list_save_files", return_value=mock_saves):
        main_menu.load_save_files()
...
def test_select_load_game(main_menu):
    ...
    with patch("main_menu.SaveGame.load_game", return_value=mock_save_data):
        main_menu.select_option()
...
def test_delete_selected_save(main_menu):
    ...
    with patch("main_menu.SaveGame.delete_save", return_value=True) as mock_delete:
        with patch("main_menu.SaveGame.list_save_files", return_value=[mock_saves[1]]):
...
def test_delete_selected_save_new_game_selected(main_menu):
    ...
    with patch("main_menu.SaveGame.delete_save") as mock_delete:
        main_menu.delete_selected_save()
...
def test_delete_adjusts_selection_when_needed(main_menu):
    ...
    with patch("main_menu.SaveGame.delete_save", return_value=True):
        with patch("main_menu.SaveGame.list_save_files", return_value=[]):
            main_menu.delete_selected_save()
