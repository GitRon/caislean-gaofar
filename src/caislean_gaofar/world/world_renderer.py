...
from caislean_gaofar.systems.combat import CombatSystem
...
from caislean_gaofar.ui.attack_effect import AttackEffectManager
...
class WorldRenderer:
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        self.dungeon_entrance_renderer = DungeonEntranceRenderer()
        ...
        for chest in entity_manager.chests:
            if camera.is_visible(chest.grid_x, chest.grid_y):
                ...
                if (
                    fog_enabled
                    and fog_of_war
                    and not fog_of_war.is_visible(chest.grid_x, chest.grid_y)
                ):
                    continue
        ...
        for monster in entity_manager.monsters:
            if monster.is_alive and camera.is_visible(monster.grid_x, monster.grid_y):
                ...
                if (
                    fog_enabled
                    and fog_of_war
                    and not fog_of_war.is_visible(monster.grid_x, monster.grid_y)
                ):
                    continue
    ...
    def _draw_dungeons_with_camera(
        ...
        for dungeon_id, (
            ...
            # Draw entrance icon using specialized renderer
            self.dungeon_entrance_renderer.draw_entrance(
                self.screen, terrain_char, x, y, size
            )
            ...
            if distance == 0:
                ...
                for spawn in dungeon_manager.world_map.get_entity_spawns("dungeons"):
                    if spawn.get("id") == dungeon_id:
                        ...
                        self.dungeon_entrance_renderer.draw_entrance_name(
                            self.screen, dungeon_name, x, y, size
                        )
                        break
